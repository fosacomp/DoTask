...
function simpleExecuteLob(sql, bindParams, processCLOB, opt) {

    var closure = {sql: sql, options: opt || {autoCommit: true}, bindParams: bindParams || {}};

    return new Promise(function(resolve, reject) {
        getConnection()
            .then(function(connection){
                execute(this.sql, this.bindParams, this.options, connection)
                    .then( function(result) {
                        Promise
                            .all(result.rows.map(processCLOB))
                            .then(function()
                            {
                                resolve(result);
                                if (closure.options.autoCommit)
...
module.exports.simpleExecuteLob = simpleExecuteLob;

function simpleExecute(sql, bindParams, opt) {

    var closure = {sql: sql, options: opt || {autoCommit: true}, bindParams: bindParams || {}};
    closure.options.maxRows = 99999;
